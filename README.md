...
`POST /api/treasures/:token/claim` atomically sets `claimed_at` and a hash of a fresh winner-only key. PostgreSQL's row lock and predicate recheck make exactly one concurrent request receive the treasure and that key, even across application replicas. Later requests get only `{state: "claimed"}`. The winner's claim response also sets a per-link, HttpOnly, SameSite=Strict recovery cookie scoped to that link's claim API, with a 30-day lifetime. The database stores only the hash of this random credential. On reload, the same browser can prove it is the winner and retrieve its treasure and name-submission credential again; other devices receive only the claimed state. Clearing cookies/site data, changing browser profiles/devices, or losing the original claim response means the recovery credential is unavailable and the treasure cannot be restored on that device. The winner can optionally submit their name to `POST /api/treasures/:token/winner`; only the claim's private key hash can authorize this write, and a link accepts one winner name. The admin view polls for names and shows a pending label until submitted. Losing claims are ranked in PostgreSQL lock order: the first distinct loser is 2nd place, the next is 3rd, and later losers receive a message selected for their claim-time gap (under 15s, 15–30s, or 30–45s). Each browser gets a per-link HttpOnly visit cookie; its hashed key and assigned rank, gap, tier, and randomly selected message are saved in `treasure_visits`, so reloads show the same result without consuming another place. These place counts start at deployment; existing links do not have historical loser ranks. A losing claim gets the nonnegative difference from the winner's `claimed_at`, calculated at PostgreSQL microsecond timestamp precision and displayed to the nearest microsecond (fractional milliseconds); this is the gap between server-side claim processing times, not a claim about device clocks or exact scan timing. The public page never embeds the secret; admin link listings never return it. The winner screen labels the prize as an OpenAI credit code, includes a copy control, advises the winner to keep it private, and provides cautious redemption guidance. Credit redemption depends on the specific offer; API credits are not ChatGPT subscriptions or API keys.
