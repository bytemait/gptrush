...
async function renderQrPng(svg: SVGSVGElement): Promise<Blob> {
  ...
  try {
    ...
    return await new Promise<Blob>((resolve, reject) => canvas.toBlob(blob => blob ? resolve(blob) : reject(new Error('Could not create the QR image.')), 'image/png'));
  } finally {
...
export default function QRDownload({ value, title, onClose }: { value: string; title: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    async function createPng() {
      ...
      if (!svg) return;
      try {
        const image = await renderQrPng(svg);
        if (!active) return;
  ...
  async function download() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Could not download this QR code.');
  ...
  return <div role="presentation" onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }} className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/60 p-4">
    <section role="dialog" aria-modal="true" aria-labelledby="qr-dialog-title" className="my-auto w-full max-w-md rounded-2xl bg-white p-5 shadow-2xl sm:p-7">
      ...
      <div className="mx-auto mt-5 w-full max-w-[320px] border border-[#ecece7] bg-white p-3">
        {imageUrl ? <img src={imageUrl} alt={`QR code for ${title}`} width={OUTPUT_SIZE} height={OUTPUT_SIZE} className="block aspect-square h-auto w-full select-auto" /> : <div aria-label="Generating QR code" className="aspect-square w-full animate-pulse bg-[#f1f2ec]" />}
        <div ref={qrRef} aria-hidden="true" className="pointer-events-none fixed -left-[10000px] top-0 h-[1024px] w-[1024px] overflow-hidden"><QRCode value={value} size={1024} level="H" title={`${title} QR code`} /></div>
      </div>
      <p className="mt-3 text-center text-[10px] text-[#898b80]">High-resolution PNG · 1024 × 1024 pixels. Right-click the image to copy it.</p>
